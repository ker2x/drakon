...
proc open { db filename type } {
	...
	if { [ info exists db_names($db) ] } {
		return [ list  {} "Database name $db is already used" ]
	}
	
	if { ![ file exists $filename ] } {
		return [ list  {} "File $filename does not exist" ]
	}
		
	if { [ catch { sqlite3 $db $filename } msg ] } {
		return [ list  {} "Could not open database $filename" $msg ]
	}
	...
	if { [ catch {
		...
		$db close
		return [ list  {} "Incompatible database $filename"  $msg]
	}
	
	if { $actual_type != $type } {
		$db close
		return [ list  {} "Wrong schema in database $filename" "type=$actual_type" ]
	}
...
proc create { db filename type version start_version script } {
	...
	if { [ info exists db_names($db) ] } {
		return "Database name $db is already used"
	}
		
	catch { file delete $filename }
		
	if { [ catch { sqlite3 $db $filename } msg ] } {
		return "Could not create database $filename"
	}
	...
	if { [ catch { 
		...
		log $msg
		$db close
		return "Could not init database $filename"
	}	
...
proc save_as { db filename } {
	...
	if { ![ info exists db_names($db) ] } {
		return "Unknown database name $db"
	}
	...
	if { [ catch { file copy -force -- $old_name $filename } ] } {
		return "Could not create or replace file $filename."
	}
  ...
  if { [ catch {
	foreach change $changes {
		...
		switch $edit {
			...
			default { error "wrong command $edit" }
		}
		#puts $sql
		$db eval $sql
	}
...
proc make_insert { rowinfo } {
	...
	if { $count < 4 } {
		error "rowinfo too short for INSERT"
	}
...
proc make_update { rowinfo } {
	set count [ llength $rowinfo ]
	if { $count < 6 } {
		error "rowinfo too short for UPDATE"
	}
...
proc make_delete { rowinfo } {
	set count [ llength $rowinfo ]
	if { $count != 4 } {
		error "rowinfo for DELETE should contain <table> <key-field> <key-name>"
	}
